function [new_model,objmax,target,growth,indmax] =  OptRAM_main(model,v,regnet,BPCYids,loc)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
...
T=T_max;
initial_size=6;
ctime=datestr(now,30);%获取当前时间,30表示时间输出格式为：‘yyyymmddTHHMMSS’
tseed=str2num(ctime((end-5):end));%set random seed,((end-5):end):就是从倒数第6个点到最后一个点的所有值，用来表示此时此刻的时分秒。
rand('seed',tseed)
...
function individual=checkTF(individual1,regnet,n_mgene)  %删掉改造方案中那些已经被TF调控的代谢基因。方案中有ABCD四个基因，如果D为TF且调控了A，A在这一步会被删除
if ~isempty(individual1.gene(individual1.gene>n_mgene))
    del=[];
    TFs=individual1.gene(individual1.gene>n_mgene);
    TFs=TFs-n_mgene;
    a=regnet.mat(:,TFs);
    gene2del=find(sum(abs(a),2)>0);%挑出随机产生的TF因子所相关的被调控基因序号（在regnet里面的序号）
    mgeneid=regnet.mgeneid;
    for i=1:numel(gene2del)
        x=find(individual1.gene==mgeneid(gene2del(i))); % mgeneid(gene2del(i))表示被调控的基因在model.gene里面的序号
        if ~isempty(x)
            del=[del x];
        end
    end
